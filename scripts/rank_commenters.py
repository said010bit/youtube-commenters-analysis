...
def rank_commenters(df, limit=100):
    ...
    commenter_stats = df.groupby(['author_name', 'author_channel_id', 'author_channel_url']).agg({
        ...
        'comment_text': lambda x: ' | '.join(str(c)[:100] for c in x[:3]),  # Sample comments
    ...
    # Fill NaN values
    commenter_stats['avg_likes_per_comment'] = commenter_stats['avg_likes_per_comment'].fillna(0)
    commenter_stats['total_likes'] = commenter_stats['total_likes'].fillna(0)
...
def main():
    ...
    try:
        ...
        print(f"Loaded {len(df)} comments from {args.input_file}")
